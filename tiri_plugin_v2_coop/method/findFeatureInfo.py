...
def run(file_glob,max_line,switch,img_col):
    try:
        ...
        for txt_name in glob.glob(file_glob2):
            ...
            if len(f_lines_num) == max_line:
                
                logging.info('the txt file with full frame: '+str(txt_name))
                ...
                graph_name = graph_name.replace(file_glob,'')
