...
def run(switch,ms_in):
    plt.switch_backend('agg')
    if switch == 0:
        folder_path ='./dbscan_degree_input/*.txt'
    if switch == 1:
    ...
    for txt_path in glob.glob(folder_path):
        ...
        clustering = DBSCAN(eps=3.5, min_samples=ms_in).fit(XX)
        ...
        if switch == 0:
            i3 = i2.replace('./dbscan_degree_input/','')
            i3 = i3.replace('.txt','')
        if switch == 1:
            i3 = i2.replace('./feature_point_1/','')
            i3 = i3.replace('.txt','')

        save_name = './dbscan_degree_output/'+'dbscan_'+i3+'_'+'5'+'_'+str(ms_in)+'.png'
        # plt.show() # for test
